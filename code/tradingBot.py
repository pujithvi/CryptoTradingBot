...
wx_client = Client(api_key=wazirx_config.API_KEY,
                   secret_key=wazirx_config.API_SECRET)
...
def fetch_data(ticker):
    ...
    try:
        bars = exchange.fetch_ohlcv(
            ticker, timeframe=f'{CANDLE_DURATION_IN_MIN}m', limit=100)
    except:
...
def execute_trade(trade_rec_type, trading_ticker):
    ...
    try:
        ticker_price_response = wx_client.send(
            "ticker", {"symbol": trading_ticker})
        if (ticker_price_response[0] in [200, 201]):
            current_price = float(ticker_price_response[1]['lastPrice'])

            scrip_quantity = round(INVESTMENT_AMOUNT_DOLLARS/current_price,
...
# runs the bot for a certain coin
def run_bot_for_ticker(ccxt_ticker, trading_ticker):
    ...
    while 1:
        ...
        if ticker_data is not None:
            ...
            print(
                f'{datetime.now().strftime("%d/%m/%Y %H:%M:%S")}  TRADING RECOMMENDATION: {trade_rec_type}')
...
# example that trades bitcoin against USD
